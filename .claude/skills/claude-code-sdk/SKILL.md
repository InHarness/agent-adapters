...
- **End-of-turn policy (success)**: right after yielding a turn's `result` (no `await` in between, so it's atomic w.r.t. consumer pushes), the adapter checks `channel.hasPending()`. Non-empty → keep the channel open; the SDK consumes the next queued message as a new turn → another `result`. Empty → `channel.close()` synchronously; the SDK's next pull resolves `done` and `execute()` ends (one-shot behavior). This pins the race window shut — a push landing after close gets `false`, and the consumer re-dispatches after-turn. No lost-message window.
- **End-of-turn policy (error)**: a failed turn (see quirk 12) **closes the channel *before* yielding the `error` event**, not after. This way a consumer that pushes synchronously in response to the error gets a clean `false` (the session is over) instead of having that push silently run as another turn against a failed session — and with nothing queued the SDK's next pull resolves `done`, so the stream actually closes. The consumer decides whether to retry/re-dispatch (e.g. via `resumeSessionId`). Messages already queued *before* the error still drain (they were legitimately accepted).
...
12. **A `result` with `subtype: 'success'` can still be a failure — check `is_error`.** `SDKResultSuccess` (`sdk.d.ts`) carries `is_error: boolean` and `api_error_status?: number | null` alongside the `subtype`. When the API errors mid-turn (e.g. a 500) but the SDK still produces a final result message, it emits **`subtype: 'success'`, `is_error: true`, `api_error_status: 500`**, with the error text in the `result` string field (`"API Error: 500 Internal server error"`). Branching on `subtype` alone (the pre-fix bug) mislabels this as a successful `result` event and, in streaming-input mode, keeps the channel open — the stream never closes. The adapter now treats a turn as failed when **`is_error === true` OR `subtype !== 'success'`** and routes it to a `type: 'error'` event (`phase: 'runtime'`).
    - Error message priority: the `errors: string[]` array carried by the *real* error subtypes (`error_during_execution`, `error_max_turns`, `error_max_budget_usd`, `error_max_structured_output_retries`) — these do NOT have a `result` field — then the `result` string (the `is_error` "success" case), then `'Unknown error'`. `api_error_status`, when present, is prefixed (`API error 500: …`).
    - See quirk 1's "End-of-turn policy (error)" for how the streaming-input channel is closed before the `error` is yielded. Tests: `src/adapters/claude-code.errors.test.ts`.
