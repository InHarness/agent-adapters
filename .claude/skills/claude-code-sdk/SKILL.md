...
- **TODO / things to watch**:
  - New thinking modes (beyond ADAPTIVE_THINKING_ONLY). Keep `src/models.ts:ADAPTIVE_THINKING_ONLY` in sync when Anthropic ships a new adaptive-only model. **Claude Fable 5 (`claude-fable-5`) added 2026-06-09** and **Claude Opus 5 (`claude-opus-5`, 1M context) added 2026-07-28** — both adaptive-only, in the set. Watch for `claude-mythos-5` (restricted-access Mythos tier; no public model ID published yet — add the alias once Anthropic documents one).
