...
- **TODO / things to watch**:
  - New thinking modes (beyond ADAPTIVE_THINKING_ONLY). Keep `src/models.ts:ADAPTIVE_THINKING_ONLY` in sync when Anthropic ships a new adaptive-only model. **Claude Fable 5 (`claude-fable-5`) added 2026-06-09**, **Claude Opus 5 (`claude-opus-5`, 1M context) added 2026-07-28** and **Claude Fable 5.1 (`claude-fable-5-1`, 1M context) added 2026-09-07** — all adaptive-only, in the set. Watch for `claude-mythos-5` (restricted-access Mythos tier; no public model ID published yet — add the alias once Anthropic documents one).
...
1. **Opus 4.6+, Sonnet 5 and Fable 5 require adaptive thinking.** `src/models.ts:ADAPTIVE_THINKING_ONLY` lists model IDs that reject fixed-budget thinking (`claude-fable-5`, `claude-sonnet-5`, `claude-opus-4-6`, `claude-opus-4-7`, `claude-opus-4-8`, `claude-opus-5`). The adapter auto-converts `{ type: 'enabled', budget_tokens: N }` into `{ type: 'adaptive' }` for those models. The rule is documented in `node_modules/@anthropic-ai/claude-agent-sdk/sdk.d.ts` (L1176-1177: `'adaptive'` is for "Opus 4.6+" and is the default for models that support it). When a new Opus/Fable/Sonnet ships, add it to the set — and to the `claude_thinking.modelOverrides` view in `src/options.ts`, which `src/models.test.ts` pins against the set so the two cannot drift.
   ...
   - **Opus 5 effort ladder (not exposed yet):** `claude-opus-5` accepts `low|medium|high|xhigh|max` (default `high`), wider than the `claude_effort` knob's `low|medium|high`. Widening it is a deliberate open question in the spec (`spec/index.md`, open question #8), not an oversight — do not add `'xhigh'`/`'max'` to `src/options.ts` without a brief. Related watch item: on Opus 5, `thinking: { type: 'disabled' }` at effort `xhigh`/`max` is a 400. Unreachable today (the adapter never emits `'disabled'`), but it constrains any future disabled-thinking path.
...
- **"Thinking events aren't showing for Opus 4.6 / 4.7"**
  ...
  → **Opus 4.7 silently changed `thinking.display` default to `'omitted'`** ([Anthropic docs](https://platform.claude.com/docs/en/build-with-claude/adaptive-thinking#controlling-thinking-display)). The adapter now passes `display: 'summarized'` automatically for any model in `ADAPTIVE_THINKING_ONLY`. Override per-call via `architectureConfig.claude_thinking_display: 'summarized' | 'omitted'`. **Fable 5 (`claude-fable-5`) and Fable 5.1 (`claude-fable-5-1`) share this `'omitted'` default** and are members of `ADAPTIVE_THINKING_ONLY`, so they get the `'summarized'` restoration automatically — no Fable-specific handling required.
