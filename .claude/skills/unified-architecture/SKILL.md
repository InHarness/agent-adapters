...
- **Subagent lifecycle**: `subagent_started { taskId, description, toolUseId, resumed? }` (`taskId` = the agent, `toolUseId` = the invocation; a re-entered agent gets a second pair for the same `taskId` with `resumed: true` — pairs sequence, never nest; teammates/peers emit nothing), `subagent_progress { taskId, description, lastToolName? }`, `subagent_completed { taskId, status, summary?, usage? }`
