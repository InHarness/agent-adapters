...
- **Verified-pin log** — a table `| date | dev-pin | verified by | findings |` recording every dev-pin this consumer has *actually* been verified against (an e2e run, or documented live use), newest row last. Three rules ride on the field: the **newest row's dev-pin must equal** the pin named in *Supported peer-SDK range* above — the two move in the same edit, so the pin has exactly one home and the log never lags it; findings are recorded **honestly** (`none observed` is a valid finding, an invented one is not); rows are **append-only** — a superseded pin's row stays, because the history of what was measured on which pin *is* the point of the field. A row may be written ahead of its verification with `findings: pending verification` when the bump is specified spec-first (see <section_ref anchor="agvf1tok"/>), and is completed once the suite has run.
