...
| `subagents` | delegated work emits `subagent_*` lifecycle with consistent `subagentTaskId`; (where supported) a consumer-defined subagent is invocable; and a subagent inherits the run's filesystem ceiling — under a soft path-scope, a subagent asked to read outside `cwd ∪ allowedPaths` is refused exactly as the parent would be (M15 — <section_ref anchor="8pg5iti3"/>); and, where the SDK lets the model re-enter a backgrounded helper, a second lifecycle pair for the same `taskId` marked `resumed`, with the session held open until it reports (M06 — <section_ref anchor="0f6287ae"/>). |
...
| `sdk-surface-probe` | Against a live engine at the current dev-pin, the set of built-ins the run actually exposes — including the deferred ones reachable only through a discovery gate — is **covered by** the adapter's known-tool inventory. A name the engine exposes and the inventory does not know is a gate that will fail closed on it at the next deny. |
...
**When a scenario may bypass the adapter.** Every scenario above drives the adapter; `sdk-surface-probe` does not, and that exemption is deliberately narrow. A scenario may bypass the adapter and call the peer SDK directly **when, and only when, what it records is evidence for that consumer's L7 verified-pin log** — L7 asks what the *engine* exposes at a pin, and an observation taken through our own mapping layer cannot answer that, because an adapter bug and an SDK change look identical from the outside. Such a scenario asserts nothing about `UnifiedEvent`; it exists to make the engine's own surface measurable. It still lives in the e2e suite, under the same skip-without-credentials contract, so that *"the e2e suite is the verified-range evidence"* (<inline_mention type="ac" slug="ac-e2e-suite-is-the-verified-range-evidence"/>) stays literally true rather than acquiring a silent exception.

<!-- anchor: agvf1tok -->
## Public API & Packaging (L4)

- **Semver** — the contract is versioned; additive `UnifiedEvent`/param changes are minor, removals/renames are major. **A surface that keeps its name and its type while its meaning changes is a major as well**, and it is the case the two rules above let through: nothing is removed and no signature moves, so it reads as additive while every consumer already calling it is now asking for something other than what it asked for before. No compiler catches that one, which is why it is settled here rather than argued per release.
...
- **Worked case — the run-bound additions are a minor, and one of them still owes a note.** Two new optional `RuntimeExecuteParams` fields (`toolCallTimeoutMs`, `subagentTimeoutMs`), one new error class (`AdapterToolCallTimeoutError`), one more entry on the outstanding-work list and a hold cap that now re-arms are all additive: nothing is renamed, nothing is removed, and a consumer setting neither field sees exactly the run it saw before. **Minor.** The exception is `collectEvents()` losing its default clock (M09 — <section_ref anchor="diq2em77"/>): the signature does not move and the surface is not even opt-in, yet a consumer that relied on the helper giving up after about two minutes now waits for the stream to end. That is the behavioral-hardening rule above running the other way — a bound withdrawn rather than one tightened — and it carries the same obligation: minor, with a CHANGELOG note naming the withdrawn default and the argument to pass in its place. Widening what re-arms that cap to a held subagent's own lifecycle (M17 — <section_ref anchor="q9u5sbot"/>) rides the behavioral-hardening rule from the other side: nothing is renamed, no signature moves, and a consumer setting no new field sees a *longer*-lived run rather than a different API — but a held subagent that used to be cut at the cap while visibly working now runs on, so it ships **minor with a CHANGELOG note** naming the widened vocabulary.
