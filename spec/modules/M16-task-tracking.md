...
**…and that recovery must not rest on one English sentence.** The identifier is announced in unstructured prose — `"Task #1 created successfully: <subject>"` verbatim on 0.3.153 and 0.3.210 — so a single reworded CLI string silently reinstates the bug above, with no error and no failing type. This is a *known weak contract*, recorded rather than papered over; the durable fix is upstream, in a structured `tool_result`. Until then the projection MUST reduce the blast radius three ways: prefer any structured identifier in the payload over prose; accept **several plausible phrasings** rather than exactly the one shipping today, rejecting captures that are obviously sentence words rather than identifiers; and keep a **deterministic positional fallback** — with exactly one create unreconciled and an update naming an id never seen assigned, the two can only be the same task. Ambiguity (two or more unreconciled) is left alone rather than guessed: a wrong alias corrupts a real item, where a missing one merely appends a stray one. A failure to resolve by any route is logged behind the debug flag, so the next regression is visible rather than silent.
