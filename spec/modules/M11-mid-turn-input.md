...
- **The run's clocks are armed per `execute()` call, not per turn.** However many turns a streaming session delivers, one backstop and one idle clock span the whole of it, so a push buys no fresh budget; what arms them, what stops them and what each expiry costs are M01's (<section_ref anchor="8do90d06"/>).
