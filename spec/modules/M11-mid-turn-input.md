...
- **The run's clocks are armed per `execute()` call, not per turn.** However many turns a streaming session delivers, one backstop and one idle clock span the whole of it, so a push buys no fresh budget; what arms them, what stops them and what each expiry costs are M01's (<section_ref anchor="8do90d06"/>). **The two span it differently, and only the backstop actually runs between pushes.** An open input channel waiting for the next push is outstanding work in M01's sense — a producer on the consumer's side is slow, the engine has not gone quiet — so the idle clock does not advance across a gap between turns, however long the consumer leaves the session open. The backstop does, exactly as stated above. What ends a session is unchanged: the channel draining, `timeoutMs`, or `abort()`.
