...
| M13 | A whole-run `timeoutMs` / `abort()` still bounds a run with in-flight background work — and ends it with M13's classes, the hold cap's own expiry included. `idleTimeoutMs` is the clock that does **not**: a background task not yet settled is outstanding work, so the idle clock does not advance while this module's work is in flight (M01 — <section_ref anchor="8do90d06"/>). |
...
- **Work that never settles / a wake-up that never arrives** → the hold must not become a hang. Holding the channel open hands the run's lifetime to the engine, which is correct while work is genuinely in flight and unbounded when it is not (a backgrounded `sleep 3600`, or an SDK version that drops the post-`result` wake-up). Adapters therefore bound the hold twice, and **neither bound may run while the model is producing** — a bound firing mid-turn closes the control channel the hold exists to protect, re-creating the very defect. So:
  ...
  - a hard **cap** on unsettled work, for a task that is still running and may never finish. It is **re-armed by every `background_task_started` / `_progress` / `_completed` belonging to a task the adapter tracks**, and expires only after that many milliseconds in which no such event arrived while something was still unsettled. Transport frames — `system/status`, `background_tasks_changed` — deliberately do **not** move it: they move the grace, which is the bound that measures engine silence. Released the moment a continuation turn begins, because from there the run is an ordinary live turn again and is bounded by `timeoutMs` / `abort()` (M13) like any other.
...
- **The cap re-arms, and that does not make it a heartbeat.** A bound that any traffic on the stream could push forward would be one, and the contract refuses that shape (M01 — <section_ref anchor="8do90d06"/>): silence carries no meaning on its own, and raw cadence is a signal no capability matrix declares. What moves this cap is a **closed vocabulary attached to one unit of work** — the `background_task_*` family of a task this adapter is tracking, whose per-adapter inequality *is* declared (<section_ref anchor="ujhcafnf"/>). The distinction is the whole value of the bound: a backgrounded `sleep 3600` says nothing about itself and is still cut at the cap, while a build reporting progress for twenty minutes is not, because the first is merely present and the second is visibly working. A cap armed once at the held `result` and never moved again cannot tell those two apart, and cuts the one that was working.
- **The cap must stay strictly under whatever timeout the consumer applies to the stream.** The two clocks race and the consumer's starts *earlier* (at the run's start; the cap only arms at a held `result`), so a cap set at or above it can never surface its own error: the consumer's clock rejects the run first, the failure is reported as a generic timeout, and the specific signal that background work was abandoned is lost. The library's own `collectEvents()` helper supplies no clock of its own (M09 — <section_ref anchor="diq2em77"/>), so there is no library-side ceiling left for an adapter default to sit under; what remains is an obligation on the **consumer**, and the levers' documentation says to raise a stream timeout and the cap together rather than either alone.
- **Deciding whether to hold.** Where the engine publishes its own in-flight report (Claude Code exposes `background_tasks` on the `Stop` hook), the adapter reads it as a **positive signal only**: non-empty means hold. An *empty* report is not permission to close — measured on 0.3.153, it reported `[]` on a session the engine then held open, woke, and ran a further turn (A01). The adapter's own task tracking stays the fallback, and the hold takes the union. A `taskId` that has already been marked *finished* and then sees a second `task_started` **re-enters** the set and **re-arms both bounds**, because the alternative is the failure this section exists to prevent: grace is armed the moment everything tracked has settled, which is exactly the window between a subagent settling and the model re-entering it, and grace expiry closes the control transport.
- A second `task_started` arrives for a `taskId` that the engine has already patched as finished but has **not** yet notified → the id is in the tracked set *and* still carries the finished mark, so "everything tracked has settled" reads true while a live cycle is running. Re-entry must clear the finished mark as well as re-adding the id; otherwise grace arms inside the re-entered cycle.
