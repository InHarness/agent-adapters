...
- **Grace expiry is silent; only the cap warns.** Settled work plus a quiet engine is how a healthy task-touching run *ends* — it is exactly where the pre-hold adapter closed the channel, at `result`. A `warning` there fires on the happy path and tells every such run that its work did not run: a false data-loss report, and a noisy one, since it lands on every run that so much as spawned a subagent. The cap is different: it can only expire with something genuinely unsettled, so its `warning` is truthful and names the bound. Both bounds degrade to the pre-hold behaviour (the run ends at `result`), so a bounded hold is never worse than no hold.
- **The cap must stay strictly under whatever timeout the consumer applies to the stream** — including the one the library's own `collectEvents()` helper applies by default. The two clocks race and the consumer's starts *earlier* (at the run's start; the cap only arms at a held `result`), so a cap set at or above it can never emit its truncation `warning`: the helper rejects the run first and the only signal that background work was abandoned is lost. Adapters pick a default below the helper's, and the lever's documentation says to raise both together.
- **A `warning` may therefore trail the terminal `result`** — it is raised from a timer, which by construction fires after the last result of the run. "Result is terminal" means *last non-`warning`, non-`flush` event* (M01/L1); conformance checks that read the literal last event are wrong.
