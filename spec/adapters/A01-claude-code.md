...
- **Background tasks (M17)** — the Bash tool's `run_in_background` parameter backgrounds the shell process (never the agent). Mechanics the adapter realizes:
  ...
  - *In-flight tracking is adapter-side, and the two kinds settle differently.* `SDKResultMessage` carries no in-flight list (the SDK exposes `background_tasks[]` on `StopHookInput` / `SubagentStopHookInput`, not on `result`), so the adapter maintains the set itself. **Both** kinds are tracked, because both can outlive the turn's `result` — a late-settling subagent is the shape the original bug report showed. They settle on different signals: a background task on its `task_notification`; a subagent on the **`tool_result` of its spawning tool call**, because a Task that finishes inside the turn emits no `task_notification` at all (observed live). Missing that second rule is not harmless: every ordinary subagent run would sit in the in-flight set and hold the channel until the cap expired. The set drives the hold below and populates M01's `result.backgroundTasks` — which therefore reports *whatever holds this run open*, subagents included, since that is what the "not end-of-run" signal has to mean. A `background_tasks[]` field found on a `result` — should a future SDK add one — is read as a union with the tracked set, never as a replacement.
  ...
  - *The hold is bounded.* A grace window after the last task settles (the continuation turn must begin within it) and a hard cap from the first held `result`; on either expiry the adapter closes the channel and emits a `warning` naming the bound, so a `sleep 3600 &` or a dropped wake-up ends the run instead of hanging it. Contract in M17 (<section_ref anchor="q9u5sbot"/>).
  ...
  - *Known gaps (unused SDK controls).* `Query.stopTask()`, `Query.backgroundTasks()`, and the `is_backgrounded` flag are not consumed — per-task stop and mid-run enumeration are unrealized; `abort()` remains the coarse stop (M13).
