...
- **Subagents (M06)** — `subagents` (definition) ignored with a one-shot `warning`; the lifecycle of Gemini's own thread-bearing tool calls is still observed and synthesized. **The warning must not claim the SDK has no subagent-definition mechanism.** At the verified pin gemini-cli-core ships an agent registry and an agent-definition shape, and registers named subagent tools of its own (<section_ref anchor="srxv89v2"/>); what this adapter does not do is translate a consumer's `SubagentDefinition` into one per call. So `subagentDefinition: false` records an **adapter** limitation, not an SDK gap, and a warning phrased as a claim about the SDK is false at this pin — even though the degradation it announces, warn-and-ignore, is correct.
- **Mid-turn (M11)** — `midTurnPush: false`; one `sendStream` per run.
- **Tool gating (M18)** — `toolGating: true` for all five groups, `delegation` included, at `soft` strength: exclusion happens in the tool registry, so a denied tool is never registered and never reaches the model — but there is no OS or server-side enforcement behind it. Consumption mechanics below; matrix in M18 (<section_ref anchor="1yllld10"/>).
...
- **Tool gating (M18)** — enforced at the tool-list level via `excludeTools`, which is applied when the tool registry is built, **before** the approval policy runs. Two consequences worth stating: an auto-approving mode (`yolo`) does **not** bypass an exclusion, since there is no registered tool left to approve; and the SDK's own `approvalMode: 'plan'` is deliberately avoided, because it makes the model defer even reads. Group→identifier mapping is canon in M18 (<section_ref anchor="1yllld10"/>); three mapping hazards live here — the `shell` group must also exclude the **background-process** tools, which are registered outside the enumerated built-in set and survive an exclusion aimed at the shell tool alone; a rename that left a legacy alias resolvable means a deny must name **both** spellings or it does not fire; and the `delegation` group's members on this adapter are the SDK's own subagent tools — `codebase_investigator`, `cli_help`, `generalist` and `confucius` — registered outside the enumerated built-in set in exactly the way the background-process tools are, and named by no group in the adapter's mapping today. That third one is a **live defect**, not an unfilled cell: one of those tools reads the codebase, so for as long as they go unnamed a `file-read` deny leaves that read reachable through a spawned helper — the laundering route opencode closes by folding its delegation bucket into `file-read`. Naming them is what puts `delegation` at `soft` here, by the same mechanism and under the same ceiling as every other group on this adapter. `excludeTools` is a **deny-only** primitive with no allow-list counterpart, which is why this adapter is capped at `soft` and cannot satisfy M18's residual-allow-list invariant: a built-in a future SDK adds is allowed until the mapping is updated (L7).
...
- **Verified-pin log** —
  ...
  | 2026-09-22 | `^0.38.0` (resolved `0.38.2`) | source sweep of the installed SDK | (1) `excludeTools` — the only gating primitive this adapter has — is marked **deprecated in favour of the Policy Engine** at this pin. The successor surfaces are the positive tool lists (`coreTools`, `mainAgentTools`, `allowedTools`) and the policy engine's own decision vocabulary (`ALLOW` / `DENY` / `ASK_USER`), and they are the **only** route by which this adapter's gating strength could ever rise from `soft` to `hard`: a call the engine refuses is refused outside the model, which a registry exclusion never is. That is why the deprecation is logged here as a measured fact rather than carried as a wish. (2) The SDK registers **named subagent tools** of its own — through an agent registry and an agent-definition shape — outside the enumerated built-in set. This is what the `delegation` mapping rests on (<section_ref anchor="oirynpg7"/>) and what makes `subagentDefinition: false` an adapter limitation rather than an SDK gap (<section_ref anchor="s7prl2pn"/>). |
