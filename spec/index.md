...
8. **Opus 5 effort ladder — expose `xhigh`/`max`?** Opus 5 (added to the M02 catalog, `claude-opus-5`, adaptive-only) ships a new reasoning-effort ladder `low|medium|high|xhigh|max` (default `high`), wider than the current `claude_effort` knob. Open: (a) whether to widen `claude_effort` to `...|'xhigh'|'max'` (an adapter/code change, downstream via brief → implementer — not spec canon today), and (b) whether the effort ladder should become **spec canon** (e.g. an L3/M02 concern) rather than living only in the adapter + the `claude-code-sdk` skill. Also a watch item: `thinking:{type:'disabled'}` at effort `xhigh`/`max` is a 400 on Opus 5 — unreachable today since the adapter never emits `'disabled'`, but it constrains any future disabled-thinking path.
