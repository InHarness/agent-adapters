...
| M17 | **Background & long-running tasks** | medium | L1, L2, L3 | `background_task_*` lifecycle (decoupled from `subagent_*`, routed by `task_type`), push resume model (`result` not terminal while work in flight — iterate to `done`), `outputFile` tail target, disable lever | `modules/M17-background-tasks.md` |
| M18 | **Built-in tool gating (deny-groups)** | high | L1, L2, L3, L4, L5, L7 | `disallowedToolGroups` over five engine-neutral `ToolGroup`s, plan-mode as a preset, per-adapter matrix with a `hard`/`soft`/`none` strength axis and canon **escape surfaces**, fail-closed refusal before dispatch, residual-allow-list invariant, composition with M15 and `autoApproveTools` | `modules/M18-tool-gating.md` |
...
Adapters consume capability-modules (each module owns its support matrix):
  ...
  ── M17 Background tasks    (A01 positive; A02–A04 absent)         (L1+L2+L3; push resume model)
  ── M18 Tool gating         (A01/A03/A04 positive; A02 partial)    (L1+L2+L3+L4+L5+L7; fail-closed)

M17 Background tasks ── decoupled from ──> M06 Subagents (shared SDK task channel split by task_type in A01)

M06 Subagents ── passes model through ──> M02 Models (a subagent's model is not resolved against the catalog)
M06 Subagents ── inherits servers from ──> M04 MCP (no per-subagent MCP config; filtered by the subagent's toolset)
M06 Subagents ── names skills from ──> M05 Skills (delivery and discovery stay M05's)
M06 Subagents ── inherits scope from ──> M15 Path-scope (a subagent's reach equals the run's)
M06 Subagents ── re-entry holds via ──> M17 Background tasks (a resumed subagent re-enters the in-flight set; the hold re-arms)

M18 Tool gating ── contends with ──> M15 Path-scope (same rule surfaces on A01; M18 deny applied last)
M18 Tool gating ── propagates into ──> M06 Subagents (a deny that stops at the parent is not a deny)
M18 Tool gating ── suppresses ──> M17 Background tasks (a background task is a shell task), M05 Skills (shell deny)
M18 Tool gating ── refuses via ──> AdapterToolPolicyError (M13); the autoApproveTools rename is semver-major (M12)
M18 Tool gating ── must preserve ──> M06 Subagents delegation tools (a deny of one group must not remove the ability to delegate)
...
1. Module budget: with M18, the spec now has **18** capability-modules — a conscious step *past* the production-hardened band (8–15), taken deliberately for M16 (Task/todo tracking), M17 (Background & long-running tasks) and now M18 (Tool gating). M18's own merge candidate — folding it into M15 as a widened "capability confinement" module — was considered and **rejected**: the two carry opposite degradation postures (M15 warns and continues, M18 refuses the run), and merging would force one posture onto both. Optional consolidation (M08+M09 → "Stream & metrics"; M10+M11 → "Prompt & mid-turn input") remains available on a separate round if the set feels heavy.
...
5. **Path-scope (M15) — deferred design decisions.** The core contract is decided (prefixes, read+write combined, permission-rules-default with OS-sandbox opt-in, `pathScope` as a bool + separate strength signal). Still open: (a) read/write **split** and **glob** input as future extensions; (b) whether A04 gemini can honor path-scope natively (`targetDir` / include-directories) or stays `warn`-degraded — needs code verification; ~~(c) the pre-run strength-confirmation channel~~ **resolved by M18**: it is a **synchronous probe**, not a startup event, because a startup event arrives too late for a consumer to decline the run — M18 specifies `probeToolGating` in that shape and M15's strength confirmation follows the same convention (<section_ref anchor="1yllld10"/>); (d) whether the capability should additionally surface "deny-expressible vs allow-only" as a first-class signal so consumers never assume an unenforceable carve-out.
6. **Generic tool-permission policies — ~~open~~ resolved as M18, in a different shape than posited.** This question asked whether to build an `onToolPermission`-style **hook** as a sibling of M15. The answer shipped as M18 (`modules/M18-tool-gating.md`) and is **declarative, not imperative**: a deny-list of engine-neutral tool groups. The hook was rejected for two reasons — it hands the consumer back exactly the per-SDK tool-shape knowledge the library exists to absorb, and it cannot answer *before* the run, so a consumer building a security boundary would learn its policy is unenforceable on the first tool call of a live stream instead of while it can still decline. The declarative form is answerable synchronously; that is what makes fail-closed possible. M18 also settles the composition-with-M15 requirement this question set, and finds the two are *not* orthogonal on A01 (<section_ref anchor="1yllld10"/>). **Still parked as a later increment, deliberately out of M18's scope:** network and rate-limit policies, per-command shell rules, MCP tool gating, and any `'ask'` interaction mode. M18 fixes the group vocabulary so these layer on without a rename.
7. **Autonomous self-scheduling loop (deferred — next design round).** A01 now *hard-suppresses* the harness scheduling family (`ScheduleWakeup`, `Cron*`, `/loop`) because it is inert under headless drive. The follow-up is to replace that dead passthrough with a **real, cross-adapter** capability: a library-owned loop that ends the turn, sleeps N seconds, and re-invokes itself with a wake-up prompt — built on primitives we already have (M07 resume, M11 mid-turn/streaming input). Likely a **future** module (`M19` — `M16`/`M17`/`M18` are taken; distinct from M17's engine-managed background shell tasks) with a degradation matrix (L2), loop config (L3: max iterations, backoff, stop-condition), cross-wake usage aggregation (M08), and a whole-cycle timeout/abort (M13). **Scope tension to resolve at the start of that round:** it directly contradicts the current *"Out of scope: background jobs — N/A for a library"* stance — a conscious decision, not a silent drift.
8. **Opus 5 / Opus 5.5 effort ladder — expose `xhigh`/`max`?** Opus 5 (`claude-opus-5`) and now Opus 5.5 (`claude-opus-5-5`), both in the M02 catalog and both adaptive-only, ship a reasoning-effort ladder `low|medium|high|xhigh|max`, wider than the current `claude_effort` knob. Opus 5.5 sharpens the question in two ways. First, the ladder is unchanged but its **default drops to `medium`** (every other model defaults to `high`), so a request that omits `claude_effort` runs one level lower than it did on Opus 5 — a silent behaviour change for any consumer that switches model without touching config. Second, the watch item hardens: `thinking:{type:'disabled'}` is a 400 at **every** effort level on Opus 5.5, not only at `xhigh`/`max` as on Opus 5 — still unreachable today since the adapter never emits `'disabled'`, but it now forecloses a disabled-thinking path entirely rather than partially. Open, unchanged: (a) whether to widen `claude_effort` to `...|'xhigh'|'max'` (an adapter/code change, downstream via brief → implementer — not spec canon today), and (b) whether the effort ladder should become **spec canon** (e.g. an L3/M02 concern) rather than living only in the adapter + the `claude-code-sdk` skill.

9. **Agent-to-agent messaging (`SendMessage` / `ListAgents`) — scoped out of the contract, deliberately.** The Claude Agent SDK ships one tool with three uses: continuing a subagent this run spawned (**in scope** — specified as subagent re-entry, M06/A01), messaging agent-team teammates, and messaging cross-session peers including **inbound** turns this run did not initiate. The last two are **not** in the unified contract: no other adapter (codex, opencode, gemini) has an equivalent, so an `agentMessaging` capability flag would be a one-adapter flag with nothing to degrade to, and the SDK surfaces it would rest on — the user-message origins `{kind:'peer', from, fromMode, verifiedPeerPid}` and `{kind:'task-notification', subkind:…}`, plus `isolatePeerMachines` — are **typed but not probed**, which L7's *declared range == verified range* discipline forbids specifying. A01 therefore pins `crossSessionInbound: 'refuse'` and reports it on `adapter_ready` (<section_ref anchor="sw3cwrsm"/>). Open: (a) whether inbound peer turns should ever surface as a `UnifiedEvent` — most likely an `origin` discriminator on `user_message` rather than a new variant; (b) whether the pinned `'refuse'` should become a consumer-settable `claude_crossSessionInbound` ArchOption, which would make it an L3 key and a resume-immutable field (M07). Probing the peer surfaces against a live second session is the precondition for either of the two.


10. **Per-call stop for the tool-call cap — deferred until all four adapters can honour it.** A `toolCallTimeoutMs` expiry ends the **run** (M01 — <section_ref anchor="8do90d06"/>). The alternative considered was to end the *call*: abandon it, hand the model a synthetic failed `tool_result`, and let the turn carry on. It was rejected on availability rather than on taste. No adapter has a per-call stop today — A01 leaves `Query.stopTask()` unconsumed and treats `Query.interrupt()` as strictly fire-and-forget on the teardown path (<section_ref anchor="sw3cwrsm"/>), and the other three expose nothing of that shape — and every channel that points *into* a run gates a call before dispatch rather than interrupting one in flight. Shipping it on one architecture would make a single field carry two different guarantees, which is the defect that settled this design elsewhere. Reopen when `Query.stopTask()` and its equivalents in the other three SDKs are both consumed and probed; until then the coarse stop is the contract.
