...
1. Module budget: with M17, the spec now has **17** capability-modules — a conscious step *past* the production-hardened band (8–15), taken deliberately first for M16 (Task/todo tracking) and again for M17 (Background & long-running tasks). Optional consolidation (M08+M09 → "Stream & metrics"; M10+M11 → "Prompt & mid-turn input") remains available on a separate round if the set feels heavy.
